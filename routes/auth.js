...
router.post("/update-info/:id", async (req, res) => {
    try {
        ...
        const { urlavatar, fullname, sex, birthday } = req.body;

        // Cập nhật thông tin người dùng trong cơ sở dữ liệu với URL của ảnh
        await UserModel.update(
            { ID: id },
            {
                fullname: fullname,
                ismale: sex === "male" ? true : false,
                urlavatar: urlavatar, // Sử dụng URL của ảnh từ phần thân của yêu cầu
                birthday: birthday,
            }
        );

        res.status(201).json({ message: "Update information successfully", urlavatar: urlavatar });
    } catch (error) {
        console.error(error);
